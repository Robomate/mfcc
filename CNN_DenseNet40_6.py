...
def batch_activ_conv(current, in_features, out_features, kernel_size, is_training, keep_prob):
  ...
  current, W, b = conv2d(current, in_features, out_features, kernel_size)
...
def block(input, layers, in_features, growth, is_training, keep_prob):
  ...
  for idx in range(int(layers)):
    tmp, W, b = batch_activ_conv(current, features, growth, 3, is_training, keep_prob)
    current = tf.concat((current, tmp),3)
    features += growth
    W_dense_list.append(W)
    b_dense_list.append(b)
    
  return current, features, W_dense_list, b_dense_list
...
#conv2d(input, in_features, out_features, kernel_size)      
current1, W1, b1 = conv2d(x_input, 1, 16, 3)    
current2, features, W2, b2 = block(current1, layers, 16, 12, is_training, keep_prob)
current3, W3, b3 = batch_activ_conv(current2, features, features, 1, is_training, keep_prob)
current4 = avg_pool(current3, 2, 'SAME')
current5, features, W4, b4 = block(current4, layers, features, 12, is_training, keep_prob)
current6, W5, b5 = batch_activ_conv(current5, features, features, 1, is_training, keep_prob)
current7 = avg_pool(current6, 2, 'SAME')
current8, features, W6, b6 = block(current7, layers, features, 12, is_training, keep_prob)    
 
current9 = tf.contrib.layers.batch_norm(current8, scale=True, is_training=is_training, updates_collections=None)
current10 = tf.nn.relu(current9)
current11 = avg_pool(current10, 8, 'SAME')
#print(current11.shape)
final_dim = features

current12 = tf.reshape(current11, [ -1, 1 * 2* 1 * 448])
#current12 = tf.reshape(current11, [ -1, final_dim ])
W_fc = weight_variable([ 1 * 2* 1 * 448, label_count ])
b_fc = bias_variable([ label_count ])
current13 = tf.matmul(current12, W_fc) + b_fc
softmax_prob = tf.nn.softmax(current13)
#print(current12.shape)  

# my own: define loss:
cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits= current13,labels = y_target))
tf.summary.scalar("cross_entropy", cross_entropy)  
         
#cross_entropy = -tf.reduce_mean(y_target * tf.log(softmax_prob + 1e-12))
l2 = tf.add_n([tf.nn.l2_loss(var) for var in tf.trainable_variables()])
optimizer = tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True).minimize(cross_entropy + l2 * weight_decay)
...
model["W_conv2"] = np.array(sess.run(W3))
model["b_conv2"] = np.array(sess.run(b3))
model["W_conv3"] = np.array(sess.run(W5))
model["b_conv3"] = np.array(sess.run(b5))
...
# 1. dense block
for num in range(len(W2)):
	model["W_dense1_"+str(num)] = np.array(sess.run(W2[num]))
	model["b_dense1_"+str(num)] = np.array(sess.run(b2[num]))
# 2. dense block
for num in range(len(W4)):
	model["W_dense2_"+str(num)] = np.array(sess.run(W4[num]))
	model["b_dense2_"+str(num)] = np.array(sess.run(b4[num]))
# 3. dense block
for num in range(len(W6)):
	model["W_dense3_"+str(num)] = np.array(sess.run(W6[num]))
	model["b_dense3_"+str(num)] = np.array(sess.run(b6[num]))
