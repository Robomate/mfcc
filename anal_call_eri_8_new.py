...
def testing(test_samples,batsize_test,buffer_test, randomint_test,directory):
	...
	for buffernum in range(int(1)):
			...
			for batch in range(2):
				...
				print()
				# loop over all hmms
				for pos3 in range(syn_num_of_nodes):
					print('pos3',pos3)
					print('==========')
					# loop over each model
					for pos4 in range (syn_no_of_refs[pos3]):
						#print(pos4)
						#print(int(syn_entry_nodes[pos3][pos4]))
						
						# start with state 1-----------------------------
						if batch == 0 and int(syn_entry_nodes[pos3][pos4]) == 1: 
							for pos5 in range(1,int(dis_max_skip[int(syn_ref_index[pos3][pos4])])+1):
								#~ print("kuku",pos3)
								#~ print("kuku",pos4)
								#~ print("kuku",pos5)
								#~ print(int(syn_ref_index[pos3][pos4]))
	                            # evtl letztes hmm noch einfügen zaehlt nur bis 64!!!
								#dis_sumprob[pos3][pos4][pos5] = dis_local_prob[int(syn_ref_index[pos3][pos4])][pos5-1] + hmm_mat['refw']['transp'][0][0][0][pos4][0,pos5]
								dis_sumprob[pos3][pos4][pos5] = dis_local_prob[int(syn_ref_index[pos3][pos4])][pos5-1] + ref_transp[int(syn_ref_index[pos3][pos4])][0,pos5]
								print(hmm_mat['refw']['transp'][0][0][0][pos4][0,pos5])  #fehlt noch
								print(dis_sumprob[pos3][pos4])
								print(dis_sumprob[pos3][pos4].shape)
								
								dis_from_frame[pos3][pos4][pos5] = -1
						...
						print('pos3',pos3)
						print('pos4',pos4)
						oldframe = dis_oldframe[pos3][pos4] #letzte eintrag stimmt evtl. nicht 
						#~ print('oldframe')
						#~ print(dis_oldframe[1][63])
						#~ print(oldframe)
						#~ print(oldframe.shape)
						local_prob = dis_local_prob[int(syn_ref_index[pos3][pos4])] #erster eintrag stimmt nicht 
						#~ print('dis_local_prob[int(syn_ref_index[pos3][pos4])]')
						#~ print(batch)
						#~ print(dis_local_prob[int(syn_ref_index[pos3][pos4])].shape)
						#~ print(dis_local_prob[int(syn_ref_index[pos3][pos4])])
						oldsum = dis_oldsum[pos3][pos4]
						#~ print('oldsum')
						#~ print(oldsum.shape)
						#~ print(oldsum)
						#transmission probs
						transp = ref_transp[int(syn_ref_index[pos3][pos4])]
						#~ print('transp')
						#~ print(transp.shape)
						#~ print(transp)
						#~ print('hmm_list[0]')
						...
						for state_ind in range(1,num_states[int(syn_ref_index[pos3][pos4])]+1):
							#print(num_states[int(syn_ref_index[pos3][pos4])]+1)
							
							
							############################################
							# start function max_search_opt
							############################################
							refliste_ind = int(syn_ref_index[pos3][pos4])
							#~ print('refliste_ind')
							#~ print(refliste_ind)
							#j = state_ind
							...
							if minskip_x > 0:
								...
								while j <= num_states[refliste_ind]+1 and minskip_x > 0:
									...
									max(dmax,dis_oldsum[pos3][pos4][j]+ ref_transp[refliste_ind][j,state_ind])  #fehlt noch index und location!!!!!!!!!
									if ind == 2:
										max_ind = state_ind - j
									j = j + 1
							...
							dis_from_frame[pos3][pos4][state_ind] = oldframe[state_ind - max_ind]	
							#~ print(oldframe[state_ind - max_ind])
							#~ print(dis_from_frame[pos3][pos4])
							dis_sumprob[pos3][pos4][state_ind] =  local_prob[state_ind -1] + dmax
				...
				for node_ind in range(syn_num_of_nodes):
					...
					for ref_ind in range(syn_no_of_refs[node_ind]):
						...
						if dis_node_prob[int(syn_entry_nodes[node_ind][ref_ind])-1] > np.log(0):
							...
							dis_from_frame[node_ind][ref_ind][0] = batch
						else:
