...
lnorm = 'no_lnorm'
USE_RNN = False
USE_BASIC_LSTM = False #True
USE_LSTM = False
USE_GRU = True # False
USE_BiLSTM = False #True
num_layers = 1
...
# for ctc: 1d array of size [batch_size]
seq_len = tf.placeholder(tf.int32, [None])

# choose cell variants:

if USE_RNN:
    cell = tf.contrib.rnn.BasicRNNCell(nodes)
elif USE_BASIC_LSTM:
    cell = tf.contrib.rnn.BasicLSTMCell(nodes, state_is_tuple=True)
elif USE_LSTM:
	cell = tf.contrib.rnn.LSTMCell(nodes,state_is_tuple=True)
elif USE_GRU:
	cell = tf.contrib.rnn.GRUCell(nodes)
elif USE_BiLSTM:
	# forward cell
    lstm_fw_cell = tf.contrib.rnn.BasicLSTMCell(nodes,state_is_tuple=True)
    # backward cell
    lstm_bw_cell = tf.contrib.rnn.BasicLSTMCell(nodes,state_is_tuple=True)
else:
    print("Error, choose other cell option")

# BiLSTM
if USE_BiLSTM:
	with tf.variable_scope("LSTM"):
		rnn_outputs, _  = tf.nn.bidirectional_dynamic_rnn( lstm_fw_cell,lstm_bw_cell,x_input, dtype=tf.float32)
		output_fw, output_bw = rnn_outputs
	# reshape rnn_outputs
	output2 = tf.reshape(rnn_outputs, [-1, nodes])
	output_fw2 = tf.reshape(output_fw, [-1, nodes])
	output_bw2 = tf.reshape(output_bw, [-1, nodes])
	rnn_outputs2 = output_bw2 #output dummy
	# linear output layer
	output_fw3 = tf.matmul(output_fw2, weight_variable([nodes, classnum]),name="w_fw")
	output_bw3 = tf.matmul(output_bw2, weight_variable([nodes, classnum]),name="w_bw")
	output = tf.add(tf.add(output_fw3,output_bw3),bias_variable([classnum]))
	output_bi = output	
	####################################################################
	#alternative via concat:
	outputsx = tf.concat(rnn_outputs, 2)
	outputs_conc = tf.reshape(outputsx, [-1, 2 * nodes])
	# linear output layer
	output_bi_conc = tf.matmul(outputs_conc, weight_variable([2 * nodes, classnum])) + bias_variable([classnum])
	#output_bi2 = tf.reshape(output_bi, [-1, batch_x_shape[0], n_hidden_6])	
########################################################################	
# other cells			
else:
	# define multi cells	
	if USE_RNN or USE_GRU:
		# stacking rnn cells
		stack = tf.contrib.rnn.MultiRNNCell([cell] * num_layers)
	elif USE_BASIC_LSTM or USE_LSTM:
		# stacking rnn cells
	...
	with tf.variable_scope("LSTM"):
		rnn_outputs, _ = tf.nn.dynamic_rnn(cell, x_input, initial_state=initial_state, dtype=tf.float32)
	with tf.variable_scope("Multi"):
		rnn_outputs_stack, _ = tf.nn.dynamic_rnn(stack, x_input, dtype=tf.float32)
...
#~ loss_ctc = tf.nn.ctc_loss(output, y_reshaped, seq_len)
#~ cost = tf.reduce_mean(loss_ctc)
#~ # Decoder, Option 2: tf.nn.ctc_beam_search_decoder
#~ decoded, log_prob = tf.nn.ctc_greedy_decoder(output, seq_len)
#~ # Accuracy: label error rate
#~ acc = tf.reduce_mean(tf.edit_distance(tf.cast(decoded[0], tf.int32),y_reshaped))
...
elif beam_search_decoder == 'greedy':
decoded, log_prob = ctc_ops.ctc_greedy_decoder(logits,seq_length, merge_repeated=False)
'''
########################################################################
...
def training(epochs,train_samples):
	...
	for epoch in range(1,epochs+1):
		...
		for minibatch in range(train_samples):
			...
			############################################################
			#bi without concat
			#~ print('sess.run(output_bi,feedtrain)')
			#~ print(sess.run(output_bi,feedtrain))
			#~ print(sess.run(output_bi,feedtrain).shape)
			############################################################
			#bi concat
			#~ print('sess.run(outputs_conc,feedtrain)')
			#~ print(sess.run(outputs_conc,feedtrain))
			#~ print(sess.run(outputs_conc,feedtrain).shape)
			#~ print('sess.run(output_bi_conc,feedtrain)')
			#~ print(sess.run(output_bi_conc,feedtrain))
			#~ print(sess.run(output_bi_conc,feedtrain).shape)
			############################################################
			#Multicell, use without Bilstm
			lstm_outx = tf.get_collection(key=tf.GraphKeys.GLOBAL_VARIABLES, scope="Multi")
			...
			print('sess.run(rnn_outputs_stack,feedtrain)')
			print(sess.run(rnn_outputs_stack,feedtrain))
			print(sess.run(rnn_outputs_stack,feedtrain).shape)
			#singlecell, use without Bilstm
			print('sess.run(rnn_outputs,feedtrain)')
			print(sess.run(rnn_outputs,feedtrain))
			print(sess.run(rnn_outputs,feedtrain).shape)
