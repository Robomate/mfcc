...
def create_minibatch_ctc(rand_mb, dict_list):
	...
	label2 = label1.replace(')', '').replace('"', '').replace('?', '').replace(',', '')
	label = label2.replace(')', '').replace('"', '').replace('?', '').replace(',', '')\
	              .replace('!', '').replace('-', ' ').replace('_', ' ')
...
all_tuple = (file_list_wav,file_list_lab)
train_list = train_loop(epochs, all_tuple, batchsize_train,char_tuple, unit_test)
