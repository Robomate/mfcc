...
with tf.name_scope(name="conv1"):
	W1 = weight_variable([4, 4, 1, amap_1]) 
	b1 = bias_variable([amap_1])
	tf.summary.histogram("weights", W1)
	tf.summary.histogram("biases", b1)   
#2. hidden layer
with tf.name_scope(name="conv2"):
	W2 = weight_variable([3, 3, amap_1, amap_2 ])
	b2 = bias_variable([amap_2 ])
	tf.summary.histogram("weights", W2)
	tf.summary.histogram("biases", b2)    
#3. hidden layer
with tf.name_scope(name="conv3"):
	W3 = weight_variable([3, 3, amap_2 , amap_3])
	b3 = bias_variable([amap_3])
	tf.summary.histogram("weights", W3)
	tf.summary.histogram("biases", b3)  
#4. hidden fc-layer
with tf.name_scope(name="fc1"):
	W4 = weight_variable([amap_3*3*3, fc_1])
	b4 = bias_variable([fc_1])
	tf.summary.histogram("weights", W4)
	tf.summary.histogram("biases", b4)   
#5. hidden fc-layer
with tf.name_scope(name="fc2"):
	W5 = weight_variable([fc_2, fc_2])
	b5 = bias_variable([fc_2])
	tf.summary.histogram("weights", W5)
	tf.summary.histogram("biases", b5)
#6. output layer
with tf.name_scope(name="fc3"):
	W6 = weight_variable([fc_3, classnum])
...
for epoch in range(1,epochs+1):
	...
	for buffernum in range(int(train_size/buffersamples)):
		...
		for batch in range(minibatches):
			...
			trainbatchlabel90 = np.rot90(trainbatchlabel,2)
			#~ print(lab90.shape)
			#~ print(trainbatchdata.shape)	
			#~ print(trainbatchlabel.shape)
			#~ print(trainbatchlabel)	
			
			'''feed minibatch into model:'''
					
			#start feeding data into the model:
			feedtrain = {x_input: trainbatchdata, y_target: trainbatchlabel90}
