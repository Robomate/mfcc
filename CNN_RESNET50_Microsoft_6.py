...
def res_block(x_in_lin, x_in_skip, ch = 64, ch2 = 1,channel_up = False):
	'''ResNet Block'''
	# init weights:                   
	W_conv1 = weight_variable([1, 1, ch2 , ch])
	b_conv1 = bias_variable([ch])
	W_conv2 = weight_variable([3, 3, ch, ch])
	b_conv2 = bias_variable([ch])
	W_conv3 = weight_variable([1, 1, ch ,4 * ch])
	b_conv3 = bias_variable([4 * ch])
	# zero padding
	input_channel = x_in_skip.get_shape().as_list()[-1]
	if channel_up is True:
		x_in_skip = tf.pad(x_in_skip, [[0, 0], [0, 0], [0, 0], [(input_channel)// 2,(input_channel) // 2]], "SYMMETRIC")	
	add_in = tf.add(x_in_lin,x_in_skip)
    # perform conv
	h_conv1 = tf.nn.relu(conv2d(add_in , W_conv1,1) + b_conv1)
	h_conv2 = tf.nn.relu(conv2d(h_conv1, W_conv2,1) + b_conv2)
	h_conv3 = tf.nn.relu(conv2d(h_conv2, W_conv3,1) + b_conv3)
	
	return [W_conv1,W_conv2,W_conv3,b_conv1,b_conv2,b_conv3] , h_conv3

# init weights:                   
W_fc = weight_variable([batsize_train * coeffs * framenum * 32 * 64, classnum])
b_fc = bias_variable([classnum])

# init placeholder:
x_input = tf.placeholder(dtype=tf.float32, shape=[None, coeffs, framenum, 1],name="input")
y_target = tf.placeholder(dtype=tf.float32, shape=[None, classnum],name="labels")
# init dummy
batsize_tf = tf.shape(x_input)[0]
h_conv00 = tf.zeros(shape=[batsize_tf, coeffs, framenum, 1],dtype=tf.float32,name="conv_zero")

#legend ResNet block: output linear, output from skip connection

# 1.ResNet block  
Weights_Rb_11, h_conv_RB_11 = res_block(x_input, h_conv00, ch = 64, ch2 = 1, channel_up = False)
Weights_Rb_12, h_conv_RB_12 = res_block(h_conv_RB_11, x_input, ch = 64, ch2 = 4*64, channel_up = False)
Weights_Rb_13, h_conv_RB_13 = res_block(h_conv_RB_12, h_conv_RB_11, ch = 64, ch2 = 4*64,channel_up = False)
# 2.ResNet block 
Weights_Rb_21, h_conv_RB_21 = res_block(h_conv_RB_13, h_conv_RB_12, ch = 2*64, ch2 = 4*64, channel_up = False)
Weights_Rb_22, h_conv_RB_22 = res_block(h_conv_RB_21, h_conv_RB_13, ch = 2*64, ch2 = 8*64, channel_up = True)
Weights_Rb_23, h_conv_RB_23 = res_block(h_conv_RB_22, h_conv_RB_21, ch = 2*64, ch2 = 8*64, channel_up = False)
Weights_Rb_24, h_conv_RB_24 = res_block(h_conv_RB_23, h_conv_RB_22, ch = 2*64, ch2 = 8*64, channel_up = False)
# 3.ResNet block  
Weights_Rb_31, h_conv_RB_31 = res_block(h_conv_RB_24, h_conv_RB_23, ch = 4*64, ch2 = 8*64, channel_up = False)
Weights_Rb_32, h_conv_RB_32 = res_block(h_conv_RB_31, h_conv_RB_24, ch = 4*64, ch2 = 16*64, channel_up = True)
Weights_Rb_33, h_conv_RB_33 = res_block(h_conv_RB_32, h_conv_RB_31, ch = 4*64, ch2 = 16*64, channel_up = False)
Weights_Rb_34, h_conv_RB_34 = res_block(h_conv_RB_33, h_conv_RB_32, ch = 4*64, ch2 = 16*64, channel_up = False)
Weights_Rb_35, h_conv_RB_35 = res_block(h_conv_RB_34, h_conv_RB_33, ch = 4*64, ch2 = 16*64, channel_up = False)
Weights_Rb_36, h_conv_RB_36 = res_block(h_conv_RB_35, h_conv_RB_34, ch = 4*64, ch2 = 16*64, channel_up = False)
# 4.ResNet block  
Weights_Rb_41, h_conv_RB_41 = res_block(h_conv_RB_36, h_conv_RB_35, ch = 8*64, ch2 = 16*64, channel_up = False)
Weights_Rb_42, h_conv_RB_42 = res_block(h_conv_RB_41, h_conv_RB_36, ch = 8*64, ch2 = 32*64, channel_up = True)
Weights_Rb_43, h_conv_RB_43 = res_block(h_conv_RB_42, h_conv_RB_41, ch = 8*64, ch2 = 32*64, channel_up = False)
# avg pool 
avg_out = avg_pool(tf.add(h_conv_RB_43,h_conv_RB_42),1)
# dense Layer
flat_out = tf.reshape(avg_out, [-1, batsize_train * coeffs * framenum * 64 * 32])
# readout Layer
dense_out = tf.matmul(flat_out, W_fc) + b_fc
