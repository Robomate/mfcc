...
def loadPatData(filename, headerbytes):
	'''load Pattern files: e.g. 39MFCC coefficients, 12byte header'''
...
def matmul(x, W, b, phase, name="fc"):
	'''matrix vector multiplication'''
	with tf.name_scope(name):
		h1 = tf.add(tf.matmul(x, W), b)
		bnorm = tf.contrib.layers.batch_norm(h1,center=True, scale=True,
		is_training=phase, decay=0.999, zero_debias_moving_mean=False)
		return bnorm
...
# define model:
layer_1 = activfunction(matmul(x_input,W1,b1,phase,name="fc1"),acttype)
layer_2 = activfunction(matmul(layer_1,W2,b2,phase,name="fc2"),acttype)
layer_3 = activfunction(matmul(layer_2,W3,b3,phase,name="fc3"),acttype)
layer_4 = activfunction(matmul(layer_3,W4,b4,phase,name="fc4"),acttype) 
layer_5 = activfunction(matmul(layer_4,W5,b5,phase,name="fc5"),acttype)
...
#for batch norm: moving mean/moving variance
	...
	for epoch in range(1,epochs+1):
		...
		for buffernum in range(train_samples):
			...
			if  buffernum%10 == 0:
				...
				#check values:
				#print(sess.run(layer_5,feed_dict=feedtrain))
...
def testing(test_samples=10,batsize_test=1,buffer_test=10, phase_bool=False,randomint_test=1):
	...
	for buffernum in range(int(test_samples)):
			...
			if  buffernum%10 == 0:
				...
				#check values
				#print(sess.run(layer_5,feed_dict=feedtest))
...
testparams_list, total_testacc, test_time, phoneme_labels  = testing(test_samples,batsize_test,buffer_test,False)
testparams_list1, total_testacc1, test_time1, phoneme_labels  = testing(test_samples,batsize_test,buffer_test,True)
# start testing: emission probs
testparams_list2, total_testacc2, test_time2, phoneme_labels = testing(test_samples2,batsize_test2,buffer_test2,False)
testparams_list3, total_testacc3, test_time3, phoneme_labels = testing(test_samples2,batsize_test2,buffer_test2,True)
...
model_name = modtype + "_"+ str(modlayer)+ "layer_bnorm_"+ str(timemarker)+"_" + str(total_testacc)[0:-9]
