...
def layer_norm(x,phase, name="lnorm"):
	'''layer norm'''
	with tf.variable_scope(name):
...
layer_bno1 = batch_norm(layer_lin1,phase, "bnorm1")
layer_out1 = activ_function(layer_bno1,acttype)

layer_lin2 = matmul(layer_out1,W2,b2,name="fc2")
layer_bno2 = batch_norm(layer_lin2,phase, "bnorm2")
layer_out2 = activ_function(layer_bno2,acttype)

layer_lin3 = matmul(layer_out2,W3,b3,name="fc3")
layer_bno3 = batch_norm(layer_lin3,phase, "bnorm3")
layer_out3 = activ_function(layer_bno3,acttype)

layer_lin4 = matmul(layer_out3,W4,b4,name="fc4")
layer_bno4 = batch_norm(layer_lin4,phase, "bnorm4")
layer_out4 = activ_function(layer_bno4,acttype)

layer_lin5 = matmul(layer_out4,W5,b5,name="fc5")
layer_bno5 = batch_norm(layer_lin5,phase, "bnorm5")
layer_out5 = activ_function(layer_bno5,acttype)
...
def training(epochs=1,train_samples=10):
	...
	ln_beta_history = np.empty(shape=[0],dtype=float)  #beta = shift
	ln_gamma_history = np.empty(shape=[0],dtype=float) #gamma = scale
	...
	#get bnorm params
	#print("beta, gamma,mov_mean, mov_var")
	...
	return train_acc_history, cost_history, crossval_history, train_time, total_trainacc, bn_params
...
#start training: 
train_acc_history, cost_history, crossval_history, train_time, total_trainacc, bn_params = training(epochs,train_samples)
...
model["ln_beta"] = bn_params[0]
model["ln_gamma"] = bn_params[1]
