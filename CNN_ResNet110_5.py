...
framenum = 17				        #number of frames: 39
...
def basic_block(input, in_features, out_features, stride, is_training, keep_prob):
  if stride == 1:
    ...
    #shortcut = tf.nn.avg_pool(input, [ 1, stride, stride, 1 ], [1, stride, stride, 1 ], 'VALID') #original
    shortcut = tf.nn.avg_pool(input, [ 1, stride, stride, 1 ], [1, stride, stride, 1 ], 'SAME')
    shortcut = tf.pad(shortcut, [[0, 0], [0, 0], [0, 0],
      [(out_features-in_features)//2, (out_features-in_features)//2]])
  current, W1, b1 = conv2d(input, in_features, out_features, 3, stride)
  current = tf.nn.dropout(current, keep_prob)
  current = tf.contrib.layers.batch_norm(current, scale=True, is_training=is_training, updates_collections=None)
  current = tf.nn.relu(current)
  current, W2, b2 = conv2d(current, out_features, out_features, 3, 1)
  current = tf.nn.dropout(current, keep_prob)
  current = tf.contrib.layers.batch_norm(current, scale=True, is_training=is_training, updates_collections=None)
  # No final relu as per http://torch.ch/blog/2016/02/04/resnets.html
  W_list = [W1, W2]
  b_list = [b1, b2]
  output_sum = current + shortcut
  return output_sum , W_list, b_list

def block_stack(input, in_features, out_features, stride, depth, is_training, keep_prob):
  W_list_sum = []
  b_list_sum = []
  current, W_list1, b_list1 = basic_block(input, in_features, out_features, stride, is_training, keep_prob)
  W_list_sum.append(W_list1)
  b_list_sum.append(b_list1)
  for _d in range(int(depth - 1)):
    current, W_list, b_list = basic_block(current, out_features, out_features, 1, is_training, keep_prob)
    W_list_sum.append(W_list)
    b_list_sum.append(b_list)
    #print(current.shape)
  return current, W_list_sum, b_list_sum
  
def avg_pool(input, s, pad ='VALID'):
  return tf.nn.avg_pool(input, [ 1, s, s, 1 ], [1, s, s, 1 ], pad)  
  
# init hyperparams
label_count = classnum   # number of classes
final_dim = 64
...
# start the model
current1, W1, b1 = conv2d(x_input, 1, 16, 3, 1)
current2 = tf.nn.relu(current1)

# dimension is 32x32x16
current3, W_list_sum1, b_list_sum1 = block_stack(current2, 16, 16, 1, 18, is_training, keep_prob)
#####block_stack(input, in_features, out_features, stride, depth, is_training, keep_prob)
current4, W_list_sum2, b_list_sum2 = block_stack(current3, 16, 32, 2, 18, is_training, keep_prob)
# dimension is 16x16x32
current5, W_list_sum3, b_list_sum3 = block_stack(current4, 32, 64, 2, 18, is_training, keep_prob)
# dimension is 8x8x64

#avg pool
#current6 = tf.reduce_mean(current5, reduction_indices=[1, 2], name="avg_pool") #original
current6 = avg_pool(current5, 8, 'SAME')
#~ print('current6')
#~ print(current6.shape)
current7 = tf.reshape(current6, [ -1, 1 * 2* 1 * final_dim])
#current12 = tf.reshape(current11, [ -1, final_dim ])
W_fc = weight_variable([ 1 * 2* 1 * final_dim, label_count ])
b_fc = bias_variable([ label_count ])
current8 = tf.matmul(current7, W_fc) + b_fc
softmax_prob = tf.nn.softmax(current8)
#print(current12.shape)  

# my own: define loss:
cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits= current8,labels = y_target))
tf.summary.scalar("cross_entropy", cross_entropy)  

optimizer = tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True).minimize(cross_entropy)
correct_prediction = tf.equal(tf.argmax(softmax_prob, 1), tf.argmax(y_target, 1))
	...
	for epoch in range(1,epochs+1):
		...
		for buffernum in range(train_samples):
			...
			for batch in range(minibatches):
				...
				# start feeding data into the model: use True, and 0.8
				feedtrain = {x_input: trainbatchdata, y_target: trainbatchlabel, lr: learnrate, is_training: True, keep_prob: 1.0}
				...
				training_loss = sess.run(cross_entropy,feed_dict=feedtrain)
				cost_history = np.append(cost_history,training_loss)
				train_accu = sess.run(accuracy,feed_dict=feedtrain)
				train_acc_history = np.append(train_acc_history,train_accu)
				if  (batch+1)%15 == 0:
					print(" -- training loss: " + str(training_loss)[0:-2]+" -- training acc: " + str(train_accu)[0:-2])
			...
			if  buffernum%10 == 0:
				...
				feedval = {x_input: valbatchdata, y_target: valbatchlabel, is_training: False, keep_prob: 1.0}
...
def testing(test_samples=10,batsize_test=1,buffer_test=10, randomint_test=1):
	...
	for buffernum in range(int(test_samples)):
			...
			for batch in range(minibatches):
				...
				# start feeding data into the model:
				feedtest = {x_input: testbatchdata, y_target: testbatchlabel, is_training: False, keep_prob: 1.0}
				predictions = accuracy.eval(feed_dict = feedtest)
				test_acc_history = np.append(test_acc_history,predictions)
				
				if test_samples < 20:
					# get emissionprobs
					softmaxprobs = sess.run(softmax_prob,feed_dict=feedtest)
...
#modelweights
model["W_conv1"] = np.array(sess.run(W1))
model["b_conv1"] = np.array(sess.run(b1))
model["W_fc"] = np.array(sess.run(W_fc))
model["b_fc"] = np.array(sess.run(b_fc))

#create loop for dense block list weights:
# 1. res block, [3x3,16] * 36
pos = 0
for num in range(len(W_list_sum1)):
	for num2 in range(len(W_list_sum1[num])):
		model["W_resblock1_"+str(pos)] = np.array(sess.run(W_list_sum1[num][num2]))
		model["b_resblock1_"+str(pos)] = np.array(sess.run(b_list_sum1[num][num2]))
		pos = pos + 1 
# 2. res block [3x3,32] * 36
pos = 0
for num in range(len(W_list_sum2)):
	for num2 in range(len(W_list_sum2[num])):
		model["W_resblock2_"+str(pos)] = np.array(sess.run(W_list_sum2[num][num2]))
		model["b_resblock2_"+str(pos)] = np.array(sess.run(b_list_sum2[num][num2]))
		pos = pos + 1 
# 3. res block [3x3,64] * 36
pos = 0
for num in range(len(W_list_sum3)):
	for num2 in range(len(W_list_sum3[num])):
		model["W_resblock3_"+str(pos)] = np.array(sess.run(W_list_sum3[num][num2]))
		model["b_resblock3_"+str(pos)] = np.array(sess.run(b_list_sum3[num][num2]))
		pos = pos + 1 	
