...
def Databuffer(buffersamples, framenum, dataset_name, path,randomint,directory):
	...
	for pos in range(buffersamples):		
		labellist, datalist = Preprocess(framenum, pos, dataset_name, path,randomint,directory)
...
# init weights:                   
W_conv1 = weight_variable([f1, f2, 1, f1_d])
b_conv1 = bias_variable([f1_d])
W_fc1 = weight_variable([248 * 1 * f1_d, f3_fc])
...
# 1. FC Layer
h_conv1_flat = tf.reshape(h_conv1, [-1, 248 * 1 * f1_d])
#print(h_conv1_flat)
h_fc1 = tf.nn.relu(tf.matmul(h_conv1_flat, W_fc1) + b_fc1)
# 2. FC Layer
h_fc2 = tf.nn.relu(tf.matmul(h_fc1 , W_fc2) + b_fc2)
# readout Layer
layer_out = tf.matmul(h_fc2, W_fc3) + b_fc3
# define classifier, cost function:
softmax = tf.nn.softmax(layer_out)
# define loss, optimizer, accuracy:
with tf.name_scope("cross_entropy"):
	cross_entropy = tf.reduce_mean(
	tf.nn.softmax_cross_entropy_with_logits(logits= layer_out,labels = y_target))
	tf.summary.scalar("cross_entropy", cross_entropy)
with tf.name_scope("train"):
	optimizer = tf.train.AdamOptimizer(learnrate).minimize(cross_entropy)
with tf.name_scope("accuracy"):
	correct_prediction = tf.equal(tf.argmax(layer_out,1), tf.argmax(y_target,1))
...
# modelweights
model["W1"] = np.array(sess.run(W_conv1))
model["W2"] = np.array(sess.run(W_fc1))
model["W3"] = np.array(sess.run(W_fc2))
model["W4"] = np.array(sess.run(W_fc3))
# biases
model["b1"] = np.array(sess.run(b_conv1))
model["b2"] = np.array(sess.run(b_fc1))
model["b3"] = np.array(sess.run(b_fc2))
model["b4"] = np.array(sess.run(b_fc3))
# activation type
