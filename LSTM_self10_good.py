...
def cell_type(ctype):
	...
	if ctype == "BASIC_RNN":
		cell_boolean = [True,False,False,False,False]
	elif ctype == "RNN":
		cell_boolean = [False,True,False,False,False]				
	elif ctype == "BASIC_LSTM":
		cell_boolean = [False,False,True,False,False]	
	elif ctype == "LSTM":
		cell_boolean = [False,False,False,True,False]		
	elif ctype == "GRU":
		cell_boolean = [False,False,False,False,True]		
	else:
		print("ERROR: wrong cell type!! instead choose:'BASIC_RNN','RNN','BASIC_LSTM','LSTM', 'GRU' ")
		cell_boolean = [False,False,False]		
	return cell_boolean
...
#choose cell: ['BASIC_RNN','RNN','BASIC_LSTM','LSTM', 'GRU']
cell_number = 2
...
cell_name = ['BASIC_RNN','RNN','BASIC_LSTM','LSTM', 'GRU']
...
if cell_boolean[0]:
	cell = tf.nn.rnn_cell.BasicRNNCell(hidden_dim,activation= act_func)
elif cell_boolean[1]:
	cell = tf.contrib.rnn.RNNCell(hidden_dim,activation= act_func)
elif cell_boolean[2]:
...
def testing(test_samples=10,batsize_test=1, phase_bool=False, randomint_test=1):
	...
	for minibatch in range(int(test_samples)):
			...
			if  minibatch%10 == 0:
				...
				print('test utterance: '+ str(minibatch)+'/'+str(test_size))
...
testparams_list2, total_testacc2, test_time2, phoneme_labels = testing(test_samples2,batsize_test2)
