...
'''=======================================================================
#Purpose: CTC Network, Acoustic Model (Speech Recognition)
#
...
def create_minibatch_ctc(minibatchsize,time_steps,coeffs,filenames):
	...
	for batch in range(minibatchsize):
		...
		label_file = np.loadtxt(label_input).astype(np.int32)
		mb_label_file.append(label_file)
		# zero padding
	...
	return minibatch_data, minibatch_label, seq_length, mb_label_file 
...
label_dir = "00_data/nn_ctc_mono/"
#label_dir = "00_data/nn_ctc/"
#label_dir = "00_data/nn_ctc_tri/"
...
time_steps = 200                   # unit over time
hidden_dim = 200     				# size of hidden layer
classnum = 45						# 1state mono: 45, triphone: 4643
classnum_ctc = classnum + 1     #labels from 0:134,+ blank == 135
...
with tf.name_scope("ctc_loss"):
	...
	loss_ctc = tf.nn.ctc_loss(y_target, logits, seq_len,preprocess_collapse_repeated=True)
...
def training(epochs,training_batches):
	...
	for epoch in range(1,epochs+1):
		...
		for minibatch in range(int(training_batches)):
			...
			minibatch_train_data, minibatch_train_label, seq_length, mb_label_file  = create_minibatch_ctc(batsize_train,time_steps,coeffs,trainset_buffer)
			...
			#~ print('loss_ctc')
			#~ print(sess.run(loss_ctc , feed_dict=feedtrain_ctc))
			
			#~ print('mb_label_file')
			#~ print(len(mb_label_file))
			#~ print(mb_label_file[0])
			
			#~ print('decoded')
			#~ print(sess.run(decoded_1 , feed_dict=feedtrain_ctc))
			#~ print(d)
			#~ print('log_prob')
			#~ print(sess.run(log_prob_1 , feed_dict=feedtrain_ctc))
			
			#~ mb_idx,mb_vals,mb_dshape = minibatch_train_label
			#~ print(mb_vals)
			...
			if  minibatch%100 == 0:
				# print out original vs. decoded values
				deco = sess.run(decoded_1[0] , feed_dict=feedtrain_ctc)
				dense_decoded = tf.sparse_tensor_to_dense(deco, default_value=-1).eval(session=sess)
				print('===========================') 
				...
				for i, seq in enumerate(dense_decoded):
					...
					print('\t Decoded:\n%s' % seq)
					org = mb_label_file[i]
					org_phoneme = np.empty(shape=[0],dtype=str)
					seq_phoneme = np.empty(shape=[0],dtype=str)
					for pos2 in range(len(mb_label_file[i])):
						org_phon = phoneme_dict[org[pos2]]
						org_phoneme = np.append(org_phoneme,org_phon)
					print('')
					print('\t Original_trans:\n%s' % org_phoneme)
					for pos in range(len(seq)):
						seq_phon = phoneme_dict[seq[pos]]
						seq_phoneme = np.append(seq_phoneme,seq_phon)
					print('\t Decoded_trans:\n%s' % seq_phoneme)
...
def testing(test_batches=10,batsize_test=1, phase_bool=False, randomint_test=1):
	...
	for minibatch in range(int(test_batches)):
		...
		# minibatch_data [Batch Size, Sequence Length, Input Dimension]
		minibatch_test_data, minibatch_test_label, seq_length, mb_label_file  = create_minibatch_ctc(batsize_test,time_steps,coeffs,testset_buffer)
...
plt.axis([0,len(cost_history),0,int(200)+1])
...
plt.axis([0,len(crossval_history),0,int(200)+1])
