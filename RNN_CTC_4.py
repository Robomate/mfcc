...
y_target2 = tf.sparse_placeholder(tf.int32)
...
if cell_boolean[0]:
	...
	#cell = tf.contrib.rnn.BasicRNNCell(num_units=hidden_dim,activation=act_func)
elif cell_boolean[1]:
...
# get softmax probs
softmax_probs = tf.nn.softmax(output)


########################################################################
#ctc

#batch_size, max_timesteps = tf.shape(x_input)[:2]

# Reshaping back to the original shape
logits = tf.reshape(output, [batsize_train, -1, classnum])
# Time major
logits = tf.transpose(logits, (1, 0, 2))

with tf.name_scope("ctc_loss"):
	loss_ctc = tf.nn.ctc_loss(y_target2, logits, seq_len)
...
# Error Rate: label error rate
ler = tf.reduce_mean(tf.edit_distance(tf.cast(decoded[0], tf.int32),y_target2))
...
def training(epochs,train_samples):
	...
	for epoch in range(1,epochs+1):
		...
		for minibatch in range(int(train_samples)):
			...
			minibatch_train_data, indices, values, shape = create_minibatch_ctc(batsize_train,time_steps,coeffs,trainset_buffer)
			...
			feedtrain_ctc = {x_input: minibatch_train_data,y_target2: tf.SparseTensorValue(indices, values, shape), seq_len: batsize_train}
