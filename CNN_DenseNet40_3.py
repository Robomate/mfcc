...
current11 = avg_pool(current10, 8)
final_dim = features
current12 = tf.reshape(current11, [ -1, final_dim ])
W_fc = weight_variable([ final_dim, label_count ])
b_fc = bias_variable([ label_count ])
softmax_prob = tf.nn.softmax( tf.matmul(current12, W_fc) + b_fc )    
	...
	for epoch in range(1,epochs+1):
		...
		for buffernum in range(train_samples):
			...
			for batch in range(minibatches):
				...
				feedtrain = {x_input: trainbatchdata, y_target: trainbatchlabel, lr: learnrate, is_training: True, keep_prob: 0.8}
				...
				print(" -- training loss: " + str(training_loss)[0:-2])
				print(" -- training acc: " + str(train_accu)[0:-2])
