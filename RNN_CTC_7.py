...
def create_minibatch_ctc(minibatchsize,time_steps,coeffs,filenames):
	...
	indices = np.zeros([minibatchsize*time_steps,2], dtype=np.int64)
	values = np.zeros([minibatchsize*time_steps], dtype=np.float32) 
	shape = np.array([minibatchsize, time_steps], dtype=np.int64)
	...
	for batch in range(minibatchsize):
		...
		indices[((batch+1)*time_steps-time_steps):(batch+1)*time_steps,0] = batch
		indices[((batch+1)*time_steps-time_steps):(batch+1)*time_steps,1] = np.arange(0,time_steps)
		values[((batch+1)*time_steps-time_steps):(batch+1)*time_steps] = label_padded
	return minibatch_data, indices, values, shape
...
time_steps = 11                    # unit over time
hidden_dim = 5     				# size of hidden layer
classnum = 135 						# 3state mono: 135, triphone: 4643
classnum_ctc = classnum + 1
...
seq_len = tf.placeholder(tf.int32, [None]) # 1d array >> [batch_size]
...
# optional: init initial hidden state
batch_size    = tf.shape(x_input)[0]
initial_state = cell.zero_state(batch_size, tf.float32)
# calculate cell output, states
with tf.variable_scope("RNN_cell"):
	rnn_outputs, rnn_states = tf.nn.dynamic_rnn(cell, x_input,seq_len, dtype=tf.float32)
	#rnn_outputs, rnn_states = tf.nn.dynamic_rnn(cell, x_input, dtype=tf.float32)
	#rnn_outputs, rnn_states = tf.nn.dynamic_rnn(cell, x_input, initial_state=initial_state,dtype=tf.float32)
...
logits = tf.reshape(output, [batsize_train, -1, classnum_ctc])
# time major
...
# option 2:
decoded_1, log_prob_1 = tf.nn.ctc_beam_search_decoder(logits,seq_len,beam_width=100,top_paths=1, merge_repeated=True)
...
def training(epochs,train_samples):
	...
	for epoch in range(1,epochs+1):
		...
		for minibatch in range(int(train_samples)):
			...
			minibatch_train_data, indices, values, d_shape = create_minibatch_ctc(batsize_train,time_steps,coeffs,trainset_buffer)
			
			print('trainset_buffer')
			print(trainset_buffer)
			print('minibatch_train_data')
			#print(minibatch_train_data)
			print(minibatch_train_data.shape)
			#~ print(minibatch_train_label.shape)
			#~ print(minibatch_train_label)
			...
			#define minibatch_ctc_data:
			#change size due to blank label????
			#https://www.tensorflow.org/api_docs/python/tf/nn/ctc_loss
			
			
			# start feeding data into the model: ctc
			#feedtrain_ctc = {x_input: minibatch_train_data,y_target: tf.SparseTensorValue(indices, values, d_shape), seq_len: [batsize_train]}
			#feedtrain_ctc = {x_input: minibatch_train_data,y_target:(indices, values, d_shape), seq_len: [batsize_train]}
			...
			#to do's!!!!!!
			#must be tuple len(3): minibatch_train_data
			#
			#seq_len: must be vector with len batchsize, contains len data
			...
			#~ print(sess.run(x_input, feed_dict={x_input: minibatch_train_data}))
			#~ print(sess.run(y_target, feed_dict={y_target: (indices, values, d_shape)}))
			#~ print(sess.run(seq_len, feed_dict={seq_len: [batsize_train]}))
			
			print(sess.run(loss_ctc, feed_dict=feedtrain_ctc).shape)
			#print(sess.run(x_input, feed_dict=feedtrain_ctc).shape)
